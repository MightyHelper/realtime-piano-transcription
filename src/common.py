...
from typing import TypedDict, Self, ClassVar
...
class DatasetEntry:
  ...
  def load_audio(self) -> MaestroAudio:
    ...
    audio_data, rate = librosa.load(config.maestro_dataset_root / self.audio_filename)
...
class MaestroDataset:
  @cached_property
  def csv(self):
    return pd.read_csv(config.maestro_dataset_csv_root)

  @cached_property
  def duration_csv(self):
    return pd.read_csv(config.maestro_duration_csv_root, index_col=0)
